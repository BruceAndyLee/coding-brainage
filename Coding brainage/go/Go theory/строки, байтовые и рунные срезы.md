...
// > rune slice length: 30
// То есть:
// - значения в байтах выходят за пределы 255 (не ascii)
// - длина среза считается в int32 - их ровно 30, столько же, сколько символов

// срез/массив рун необходимо явно приводит к строке
fmt.Printf("%s\n", string(runeString))
fmt.Printf("%s\n", len(string(runeString)))
// > Строка с юникодовыми символами
// > 57 - что странно, как будто юникод врубается только там, где символ выходит за пределы ascii
