...
```Go
type MappableArray struct {
	value []int
}

func (arr SugaryArray) Map(f func(int, int) int) MappableArray {
	res := make([]int, len(arr.value))
	for ind, el := range arr.value {
		res[ind] = f(arr.value[ind], ind)
	}
	return MappableArray{value: res}
}

arr := MappableArray{value: make([]int, 5)}
