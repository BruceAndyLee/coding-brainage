...
- код считывает нажатие на кнопку
- код меняет состояние отдельного пина, контролирующего состояние светодиода

Вариант с подсказкой препода:
- учитываем только переход от нажатой кнопки к отпущенной кнопке
...
bool pullupButtonPressedBefore = false;
bool lightOn = false;
...
void loop() {
  ...
  bool pullupButtonPressedCurrently = digitalRead(READ_BUTTON_PIN) == 0;
  if (!pullupButtonPressedCurrently && pullupButtonPressedBefore) {
    lightOn = !lightOn;
    digitalWrite(LIGHT_PIN, lightOn ? 1 : 0);
  }
  pullupButtonPressedBefore = pullupButtonPressedCurrently;
  delay(LOOP_DELAY_MS);
}
```


Сырой вариант без препода. Используется что-то вроде throttle.
Работает херово:
- есть задержка
- очень нестабильно работает на длинных нажатиях
```C
unsigned int initialCooldown = 400;
unsigned int LOOP_DELAY_MS = 100;
